...
-- `plan` is a label only — 'monthly' | 'quarterly' | 'lifetime' — nothing
-- reads it to decide behaviour. `expires_at` is what auth middleware
-- actually checks (see src/middleware/auth.js): NULL means never expires
-- (lifetime), otherwise a license stops authenticating the moment this
-- timestamp passes, same table/same check for both the buyer's FXServer
-- and their browser dashboard login — one expiry, everywhere.
--
-- `discord_guild_id` is an anti-sharing binding, not a purchase-time
-- field: it starts NULL and gets locked to whatever guild ID the
-- buyer's FXServer first reports on a successful requireServerAuth call
-- (middleware/auth.js) — a key+secret pair alone could otherwise be
-- copy-pasted onto a totally different Discord community's server.
-- Every later request from a different guild ID is rejected. NULL means
-- "not bound yet", not "no restriction".
CREATE TABLE IF NOT EXISTS licenses (
    ...
    plan              TEXT NOT NULL DEFAULT 'lifetime', -- 'monthly' | 'quarterly' | 'lifetime'
    expires_at        TEXT, -- NULL = never (lifetime); ISO datetime otherwise
    discord_guild_id  TEXT, -- NULL = not yet bound to a Discord server
