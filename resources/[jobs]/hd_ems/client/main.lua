...
--  uk_uhsjob's /revive [id] — no timer, no self-service respawn.
-- ═══════════════════════════════════════════════════════════════════
...
local function IsAmbulanceOnDuty()
    ...
    return pd and pd.job and pd.job.name == Config.AmbulanceJob and pd.job.onduty
end
...
RegisterNetEvent('baseevents:onPlayerDied', function(_, coords)
    ...
    while not HasAnimDictLoaded('combat@damage@writhe') do Wait(10) end
...
-- Staff /revive (uk_uhsjob's server/revive.lua) fires this exact event
-- client-side as its own "let a downed script clear its state" hook.
RegisterNetEvent('qb-ambulancejob:client:Revive', function()
    if not downed then return end
    StandUp(200)
    TriggerServerEvent('hd_ems:server:externalRevive')
end)
