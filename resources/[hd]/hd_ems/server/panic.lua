...
RegisterNetEvent('hd_ems:server:panic', function(coords, streetLabel)
    local src = source
    local ok, Player = EMS.IsOnDutyAmbulance(src)
    if not ok then
        TriggerClientEvent('HD:Client:Notify', src, 'You must be on-duty ambulance to use the panic button.', 'error')
        return
    end
