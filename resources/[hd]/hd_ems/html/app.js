...
function GetParentResourceNameSafe() {
  try { return GetParentResourceName(); } catch (e) { return 'hd_ems'; }
}
...
window.addEventListener('message', (event) => {
  ...
  if (data.action === 'show') {
    ...
    $('brand-title').textContent = data.department || 'UK Health Service';
...
async function loadArmoury() {
  ...
  $('armoury-rank').textContent = `Rank: ${res.rankLabel}`;

  $('armoury-items').innerHTML = (res.items || []).map(i => `
...
$('draw-loadout-btn').addEventListener('click', async () => {
...
$('return-vehicle-btn').addEventListener('click', async () => {
