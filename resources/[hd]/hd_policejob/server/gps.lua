...
--  Companion to hd_ems/server/gps.lua: tracks/pushes ONLY this
--  resource's own job ("police"). Deliberately reuses hd_ems's
--  existing client-side event names (ukhs:client:gpsUpdate /
--  ukhs:client:gpsRemove — kept under their old uk_uhsjob-era names)
--  — that renderer is a plain client_script, already running on every
--  connected client regardless of job, and already color-codes a unit
--  blue when jobName == 'police'. Piggybacking on it means officers
--  show up on the map with zero new client-side blip code, and it's
--  exactly the extension point hd_ems/config.lua's own GPS comment
--  describes.
-- ═══════════════════════════════════════════════════════════════════
