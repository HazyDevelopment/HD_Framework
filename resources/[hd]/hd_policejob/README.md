...
- **Live GPS**, companion to `hd_ems`: this resource only ever
  tracks/pushes its own job (`police`). It deliberately reuses
  `hd_ems`'s client-side blip renderer (`ukhs:client:gpsUpdate` /
  `ukhs:client:gpsRemove` — kept under their old `uk_uhsjob`-era event
  names on purpose, see `hd_ems/config.lua`'s GPS section) instead of
  ...
  That's exactly the extension point `hd_ems/config.lua`'s own GPS
  comment describes ("together the two give full mutual visibility").
  No new client code was needed for police blips to show up.
...
Already wired into `server.cfg`/`server.cfg.example`, in the jobs
section. Give a character the job with the framework's existing admin
command: `/setjob <serverId> police <grade 0-15>`.
...
- `Config.GPS` — `TrackableJobs` should stay `{ 'police' }` here (let
  `hd_ems` track `'ambulance'`); tune `ViewerJobs`/`UpdateInterval`.

## Related fixes made alongside this

While building this, two real, pre-existing gaps in the ambulance job
(at the time still the separate `uk_uhsjob` resource, since merged
into `hd_ems`) surfaced and were fixed:

- **Its `Bridge.GiveItem`** was calling `Player.Functions.AddItem`,
  which doesn't exist on `HD_Framework`'s real `Player` object (it only
  has `Money`/`Job`/`MetaData`/`Charinfo`/`Save`) — items live in
  `hd_inventory`, not on the core player object. Fixed to call
  `exports['hd_inventory']:AddItem` instead, so paramedics' armoury
  draws actually give them their kit (`hd_ems/server/armoury.lua` now
  does this directly, no bridge layer).
- **`HD_Framework/shared/items.lua` was missing `trauma_kit`**, which
  its grade 6–9 loadouts reference — drawing it would have silently
  failed. Added.
