...
-- ═══════════════════════════ PERMANENT JOB BLIPS ══════════════════════
-- These jobs need no application/whitelist — anyone can /setjob into one
-- and go on duty — so unlike hd_policejob/hd_emsjob (which only blip a
-- station to their own on-duty staff), every civilian job here gets a
-- static, always-visible blip at its depot (or `office` for the two
-- on-foot jobs with no depot) for every player, regardless of whether
-- they currently hold that job. One sprite/colour per job so the map
-- reads as a job directory, not a wall of identical icons.
local JobBlipStyle = {
    taxi       = { sprite = 198, colour = 5 },
    hgv        = { sprite = 477, colour = 47 },
    postal     = { sprite = 351, colour = 3 },
    binman     = { sprite = 318, colour = 26 },
    busdriver  = { sprite = 513, colour = 3 },
    reporter   = { sprite = 122, colour = 1 },
    realestate = { sprite = 374, colour = 2 },
}

CreateThread(function()
    for jobKey, jobCfg in pairs(Config.Jobs) do
        local point = jobCfg.depot and jobCfg.depot.coords or jobCfg.office
        if point then
            local style = JobBlipStyle[jobKey] or { sprite = 478, colour = 0 }
            local b = AddBlipForCoord(point.x, point.y, point.z)
            SetBlipSprite(b, style.sprite)
            SetBlipColour(b, style.colour)
            SetBlipScale(b, 0.8)
            SetBlipAsShortRange(b, true)
            BeginTextCommandSetBlipName('STRING')
            AddTextComponentString(jobCfg.label)
