...
RegisterNetEvent('hd_inventory:server:useItem', function(req)
    ...
    -- def.consumable = false is the escape hatch for reusable tools
    -- (the mechanic tablet) that shouldn't vanish after one use.
    if def.consumable ~= false then
...
-- Dragging an item onto the NUI's middle-panel Use button (as opposed
-- to right-click > Use, which is always exactly 1) — lets a quantity
-- be set first. Removes the whole amount from the stack in one write
-- (not N separate saves) but still fires onItemUsed once per unit, so
-- a listener like hd_ems's revive kit or hd_mechanic's field repair
-- genuinely applies its effect that many times. Capped at 20 regardless
-- of what's typed in, both as a sanity clamp and so one drag can't
-- queue up an unreasonable burst of side effects.
RegisterNetEvent('hd_inventory:server:useItemAmount', function(req)
    local src = source
    local ctx = OpenContext[src]
    if not ctx or type(req) ~= 'table' then return end
    local ref = ctx[req.side]
    ...
    if not item then return end
    local def = GetItemDef(item.name)
    if not def or not def.useable or def.type == 'weapon' then return end -- weapon toggle-equip doesn't have a meaningful "amount"

    local amount = math.max(1, math.min(tonumber(req.amount) or 1, item.amount, 20))

    if def.consumable ~= false then
        RemoveItemFromData(data, item.name, amount)
        SaveContainer(ref, data)
        PushContainer(src, req.side, ref, data, Config.MaxSlots, Config.MaxWeight)
    end
    TriggerClientEvent('HD:Client:Notify', src, ('Used %dx %s.'):format(amount, def.label), 'success')
    for _ = 1, amount do
        TriggerEvent('hd_inventory:server:onItemUsed', src, item.name)
    end
end)

-- ═══════════════════════════ GIVE TO CLOSEST PLAYER ════════════════════
-- Dragging an item onto the NUI's middle-panel Give button. The target
-- is never trusted from the client — re-derived here from real ped
-- coordinates, same as every other proximity-gated action in this
-- build (hd_fines' TargetRadius, hd_mechanic's shop check, etc.).
RegisterNetEvent('hd_inventory:server:giveClosest', function(req)
    local src = source
    local ctx = OpenContext[src]
    if not ctx or type(req) ~= 'table' then return end
    local ref = ctx[req.side]
    ...
    if not item then return end
    local amount = math.max(1, math.min(tonumber(req.amount) or item.amount, item.amount))

    local srcPed = GetPlayerPed(src)
    if not srcPed or srcPed == 0 then return end
    local srcCoords = GetEntityCoords(srcPed)

    local closestSrc, closestDist = nil, Config.GiveDistance
    for _, candStr in ipairs(GetPlayers()) do
        local cand = tonumber(candStr)
        if cand and cand ~= src then
            local candPed = GetPlayerPed(cand)
            if candPed and candPed ~= 0 then
                local dist = #(srcCoords - GetEntityCoords(candPed))
                if dist <= closestDist then closestSrc, closestDist = cand, dist end
            end
        end
    end

    if not closestSrc then
        TriggerClientEvent('HD:Client:Notify', src, 'No one nearby to give that to.', 'error')
        return
    end

    local targetCitizenid = GetCitizenId(closestSrc)
    if not targetCitizenid then return end

    local targetRef = { type = 'player', id = targetCitizenid }
    local targetData, targetMaxSlots, targetMaxWeight = LoadContainer(targetRef)
    local ok = AddItemToData(targetData, targetMaxSlots, targetMaxWeight, item.name, amount, item.metadata)
    if not ok then
        TriggerClientEvent('HD:Client:Notify', src, "They can't carry that much.", 'error')
        return
    end

    if amount >= item.amount then data[slot] = nil else item.amount = item.amount - amount end
    SaveContainer(ref, data)
    SaveContainer(targetRef, targetData)
    PushContainer(src, req.side, ref, data, Config.MaxSlots, Config.MaxWeight)
    if OpenContext[closestSrc] then PushSide(closestSrc, 'left') end -- live-refresh their grid too, if it happens to be open

    local def = GetItemDef(item.name)
    TriggerClientEvent('HD:Client:Notify', src, ('Gave %dx %s.'):format(amount, def.label), 'success')
    TriggerClientEvent('HD:Client:Notify', closestSrc, ('Received %dx %s.'):format(amount, def.label), 'info')
end)
...
-- Deposits items directly into a stash with no player context at all —
-- e.g. hd_mechanic crediting an ordered parts delivery straight into
-- the shop's parts stash rather than handing it to a specific player.
-- Creates the stash (via the same EnsureStash every other stash path
-- uses) if it doesn't exist yet. Returns false if the stash is full or
-- overweight — callers should treat that as "delivery failed", not
-- silently succeed.
exports('AddItemToStash', function(id, itemName, amount, label, slots, weight)
    id = tostring(id):sub(1, 50)
    EnsureStash(id, tostring(label or 'Stash'):sub(1, 60), slots, weight)
    local data, maxSlots, maxWeight = LoadContainer({ type = 'stash', id = id })
    local ok = AddItemToData(data, maxSlots, maxWeight, itemName, amount or 1)
    if ok then SaveContainer({ type = 'stash', id = id }, data) end
    return ok
end)
