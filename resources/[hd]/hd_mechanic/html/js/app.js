...
// No shop/canService gate here on purpose — ordering parts works from
// anywhere an on-duty mechanic can reach the tablet, see
// server/shop.lua's orderParts handler for why.
function renderParts(parts) {
    const list = document.getElementById('partsList');
    const catalogue = (parts && parts.catalogue) || [];

    document.getElementById('partsHint').textContent = parts && parts.stashLabel
        ? `Delivered straight to: ${parts.stashLabel}`
        : 'Delivered straight to the parts stash.';

    list.innerHTML = catalogue.map((p) => `
        <div class="parts-row">
            <span class="parts-label">${p.label} <span class="parts-amount">×${p.amount}</span></span>
            <button class="btn btn-sm btn-order" data-item="${p.item}">£${p.price}</button>
        </div>
    `).join('');

    list.querySelectorAll('.btn-order').forEach((btn) => {
        btn.addEventListener('click', () => {
            post('orderParts', { item: btn.dataset.item });
            toast(`Ordering ${btn.parentElement.querySelector('.parts-label').textContent.trim()}…`);
        });
    });
}
