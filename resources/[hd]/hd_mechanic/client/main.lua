...
-- Body-health equivalent of setEngineHealth above, driven by field
-- repairs (server/fieldrepair.lua) — 'add' clamps at full instead of
-- letting a material push it past 1000.
RegisterNetEvent('hd_mechanic:client:setBodyHealth', function(netId, mode, value)
    local veh = NetToVeh(netId)
    if not IsPlayerNearVehicle(veh) then return end

    NetworkRequestControlOfEntity(veh)
    if mode == 'add' then
        SetVehicleBodyHealth(veh, math.min(1000.0, GetVehicleBodyHealth(veh) + value))
    else
        SetVehicleBodyHealth(veh, value + 0.0)
    end
end)

RegisterNetEvent('hd_mechanic:client:fixOneTyre', function(netId)
    local veh = NetToVeh(netId)
    if not IsPlayerNearVehicle(veh) then return end
...
RegisterNUICallback('orderParts', function(data, cb)
    TriggerServerEvent('hd_mechanic:server:orderParts', data.item)
    cb({})
end)
