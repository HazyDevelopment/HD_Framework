(function () {
    ...
    const $panicCallsign = document.getElementById('panicCallsign');
    const $panicName = document.getElementById('panicName');
    const $panicLocation = document.getElementById('panicLocation');
    let panicCoords = null;
    let panicHideTimer = null;
    ...
    // ═══════════════════════════ PANIC ALERT ═══════════════════════════
    // Description (built server-side in hd_policejob/hd_ems's /panic
    // handler) is "<callsign> — <name>" — split back apart here rather
    // than sending three separate fields, since call.title/description
    // already have to carry the normal call-card rendering too.
    function showPanicAlert(call) {
        const parts = String(call.description || '').split(' — ');
        $panicCallsign.textContent = parts[0] || '—';
        $panicName.textContent = parts[1] || '—';
        $panicLocation.textContent = call.locationLabel || (call.coords ? `${call.coords.x.toFixed(1)}, ${call.coords.y.toFixed(1)}` : '—');
        panicCoords = call.coords || null;
        $panicOverlay.classList.remove('hidden');
        clearTimeout(panicHideTimer);
        panicHideTimer = setTimeout(hidePanicAlert, 20000);
    }
    function hidePanicAlert() {
        $panicOverlay.classList.add('hidden');
        clearTimeout(panicHideTimer);
    }
    document.getElementById('panicDismiss').addEventListener('click', hidePanicAlert);
    document.getElementById('panicWaypoint').addEventListener('click', () => {
        if (panicCoords) post('setWaypoint', { coords: panicCoords });
    });
    ...
    window.addEventListener('message', (event) => {
        ...
        switch (d.action) {
            ...
            case 'panicAlert':
                showPanicAlert(d.call);
                playAlert();
                setTimeout(playAlert, 250); // a second pip right after — reads as more urgent than a normal single-call alert
                break;
        }
    });
